...
namespace VoyageForge.ForgeCLR.Editor
{
    ...
    /// 可通过修复功能保存设置并触发验证。
    /// </summary>
    public sealed class DllCopyDirectoryNameCheck : IForgeCLRValidationCheck<HCLRRuntimeConfigSO>
    {
        ...
        {
            var valid = config != null && ForgeCLRValidationHelper.IsValidFolderName(config.DllCopyDirectoryName);
            return new ForgeCLRValidationItem(Title,
                valid ? $"DLL 拷贝根目录名有效：{config?.DllCopyDirectoryName}" : "DLL 拷贝根目录名不能为空，也不能包含路径分隔符",
