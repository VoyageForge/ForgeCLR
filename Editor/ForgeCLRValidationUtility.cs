...
namespace VoyageForge.ForgeCLR.Editor
{
    ...
    public static class ForgeCLRValidationUtility
    {
        ...
        private static void AppendAndroidGraphicsCheck(List<ForgeCLRValidationItem> items)
        {
            ...
            bool hasOpenGles = graphicsApis.Any(api => api == GraphicsDeviceType.OpenGLES2 || api == GraphicsDeviceType.OpenGLES3);
            items.Add(new ForgeCLRValidationItem(
                "Android 图形 API",
                hasOpenGles ? "Android 图形 API 已包含 OpenGLES，可配合 -force-gles 进行真机资源测试" : "Android 图形 API 未包含 OpenGLES，如需要 -force-gles 测试请在 Player Settings 中补充",
                hasOpenGles ? ForgeCLRValidationStatus.Passed : ForgeCLRValidationStatus.Warning));
        }
